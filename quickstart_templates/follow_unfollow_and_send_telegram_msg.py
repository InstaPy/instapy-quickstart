...
def follow():
    ...
    requests.get(
        "https://api.telegram.org/bot<INSERT_BOT_API_KEY_HERE/sendMessage?chat_id=<>INSERT_CHATID_HERE>&text='InstaPy Follower Started @ {}'".format(
        ...
        while counter < 5:
            ...
            try:
                ...
                session.follow_user_followers(
                    ["donya", "arat.gym"], amount=5, randomize=False
                )
    ...
    # Send notification to my Telegram
    requests.get(
        "https://api.telegram.org/bot<INSERT_BOT_API_KEY_HERE/sendMessage?chat_id=<>INSERT_CHATID_HERE>&text='InstaPy Follower Stopped @ {}'".format(
            datetime.now().strftime("%H:%M:%S")
        )
    )
...
def unfollow():
    ...
    with smart_run(session):
        try:
            ...
            # actions
            session.unfollow_users(
                amount=600, allFollowing=True, style="RANDOM", sleep_delay=450
            )
