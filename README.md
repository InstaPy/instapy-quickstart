...
Change the files with your account and build the image. At the project main folder:

```bash
docker image build --tag instapy_quickstart .
```

Run a container from the newly created image:

```bash
docker container run --name my_container instapy_quickstart 
```

To restart an already created container:
